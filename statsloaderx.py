...
class MixParser(object):
    ...
    def clean(self):
        ...
        document = lxml.html.document_fromstring(
            self.raw_data[(start - 1):len(self.raw_data) - 3])
    ...
    def encoding(self):
        ...
        for idx, item in enumerate(spl):
            if item.startswith('\\u'):
                spl[idx] = (item[0:6]).decode(
                    'unicode_escape').encode('utf-8') + item[6:]
        ...
        for i in xrange(len(data) - 1):
            ...
            if '-' in data[i].split('|')[0] and len(data[i].split('|')) > 2:
                ...
                problems_info.append(
                    Problem(num, solver, label, timestamp, lang, status))
...
def collect_user_info(_uid, _num_threads, _pc):
    ...
    result_pages = [el[0]
                    for el in loadme.load(get_url_range(_uid, _pc), _num_threads)]
    user_submits = sorted([item for subl in [MixParser(page).parse(
    ) for page in result_pages] for item in subl], key=operator.attrgetter('timestamp'))
...
def get_user_name(uid):
    target_url = 'http://informatics.mccme.ru' + \
        '/moodle/user/view.php?id={}'.format(uid)
    data = requests.get(target_url, headers={'User-Agent': 'Mozilla/5.0'})
    document = lxml.html.document_fromstring(data.content)
    label = document.xpath("//h2[@class='main']/text()")
    if label:
        if label[0].encode('latin1').decode('utf-8') == \
                u'Регистрационная запись пользователя была удалена':
            return 'Deleted'
        return label[0].encode('latin1').decode('utf-8').strip()
    else:
        disabled = document.xpath(
            "//div[@class='box errorbox errorboxcontent']/text()")
        if disabled:
            if unicode(disabled[0], 'utf-8') == u'No such user in this course':
                return 'Disabled'
            else:
                raise Exception('Can\'t indentify user.')
